...
public class RenderableMesh implements OpenGLRenderable{
	...
	public void render(final OpenGLGraphics graphics) {
		if( this.mesh.gpu_buffer == null || this.mesh.ibo_buffer == null )
			return;
		...
		try {
			...
			ShortBuffer ibo_data = graphics.memory_manager.prepareIndexData( this.mesh.ibo_buffer );
			...
			FloatBuffer vbo_data = graphics.memory_manager.prepareVertexData(this.mesh.gpu_buffer);
			...
			GL20.glVertexAttribPointer( in_Position.getId(), 3, GL11.GL_FLOAT, false, this.mesh.ELEMENTS_PER_VERTEX*Float.BYTES, 0 );
			GL20.glEnableVertexAttribArray( in_Normal.getId() );    //We like submitting texcoords on stream 2 for no special reason
			GL20.glVertexAttribPointer( in_Normal.getId(), 3, GL11.GL_FLOAT, false, this.mesh.ELEMENTS_PER_VERTEX*Float.BYTES, 3*Float.BYTES );
			GL20.glEnableVertexAttribArray( in_TexCord.getId() );    //We like submitting texcoords on stream 2 for no special reason
			GL20.glVertexAttribPointer( in_TexCord.getId(), 2, GL11.GL_FLOAT, false, this.mesh.ELEMENTS_PER_VERTEX*Float.BYTES, 6*Float.BYTES );
...
//			GL11.glBindTexture( GL11.GL_TEXTURE_2D, tid.get(0) );
			...
			// Draw everything
			GL11.glDrawElements(GL11.GL_TRIANGLES, this.mesh.ibo_buffer.length, GL11.GL_UNSIGNED_SHORT, 0);
